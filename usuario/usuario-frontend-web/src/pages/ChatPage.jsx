...
import { useNavigate, useParams, Link } from 'react-router-dom'; // Importar hooks de router
...
export default function ChatPage() {
        ...
        const fetchUserChats = async () => {
            ...
            try {
                ...
                if (error.response?.status === 403) {
                    console.error('[ChatPage] Error 403: Acceso denegado. Verificar rol de usuario.');
                    console.error('[ChatPage] Rol actual del usuario:', user?.rol || 'No definido');
                }
            }
        };
        fetchUserChats();
    ...
    useEffect(() => {
        const fetchChatMessages = async (chatId) => {
            try {
                ...
                console.error('[ChatPage] Error al cargar los mensajes del chat:', error);
                if (error.response?.status === 403) {
    ...
    useEffect(() => {
        ...
        const client = new Client({
                ...
                client.subscribe(`/user/queue/messages`, (msg) => {
                    ...
                    setMessages((prevMessages) => {
                        ...
                        if (isMyMessage) {
                            ...
                            const recentDuplicate = prevMessages.some(m => 
                                m.content === newMessage.content && 
                                m.senderEmail === newMessage.senderEmail &&
                                Math.abs(new Date(m.timestamp || m.sentAt) - new Date(newMessage.timestamp || newMessage.sentAt)) < 2000
                            );
            ...
            onStompError: (frame) => console.error('[ChatPage] Error de STOMP:', frame),
            onWebSocketError: (event) => console.error('[ChatPage] Error de WebSocket:', event),
        });
    ...
    const searchUsers = async (email) => {
        ...
        try {
            ...
            const response = await api.get(`/api/usuarios/search?email=${encodeURIComponent(email)}`);
    ...
    const startChatWithUser = async (targetUser) => {
        try {
            const currentUserEmail = user?.correo || user?.email;
            console.log('[ChatPage] Iniciando chat con:', targetUser.correo || targetUser.email);
    ...
    const sendMessage = () => {
        ...
        const otherParticipant = selectedChat.participants?.find(p => p.email !== currentUserEmail);
        ...
        console.log('[ChatPage] Enviando mensaje:', messageForBackend);
        ...
        const tempMessage = {
            ...
            sentAt: new Date().toISOString() // Para compatibilidad con el backend
        };
        ...
        <div className="chat-whatsapp-layout">
            <div className="chat-sidebar">
                ...
                <div className="chat-search-container">
                    ...
                    />
                    {isSearching && <div className="chat-search-loading">Buscando...</div>}
                            ...
                            {searchResults.map(searchUser => (
                                ...
                                >
                                    <span className="chat-sidebar-avatar">
                                        {searchUser.nombre ? searchUser.nombre[0].toUpperCase() : (searchUser.correo || searchUser.email)[0].toUpperCase()}
                                    </span>
                ...
                <div className="chat-sidebar-list">
                    {chats.map(chat => {
                        const currentUserEmail = user?.correo || user?.email;
                        // Buscar el otro participante usando la estructura real del ChatDTO
                        const otherParticipant = chat.participants?.find(p => p.email !== currentUserEmail);

                        return (
                            <div
                                key={chat.roomId}
                                className={`chat-sidebar-item${selectedChat?.roomId === chat.roomId ? ' selected' : ''}`}
                                // Al hacer clic, navegamos a la URL del chat
                                onClick={() => navigate(`/chat/${chat.roomId}`)}
                            >
                                <span className="chat-sidebar-avatar">
                                    {otherParticipant?.nombre ? otherParticipant.nombre[0].toUpperCase() : '?'}
                                </span>
                                <span className="chat-sidebar-name">
                                    {otherParticipant?.nombre || otherParticipant?.email || 'Usuario Desconocido'}
                                </span>
                            </div>
                        );
                    })}
                    ...
                    <div className="chat-header-left">
                        {selectedChat && (
                            <span className="chat-user-name">
                                {selectedChat.participants?.find(p => p.email !== (user?.correo || user?.email))?.nombre || 'Usuario'}
                            </span>
                        )}
                    </div>
                </div>
                    ...
                    <>
                        <div className="chat-messages-box">
                            {messages.map((msg, i) => {
                                ...
                                return (
                                    <div key={i} className={`chat-message-row ${isOwn ? 'chat-row-own' : 'chat-row-other'}`}>
                        ...
                        <div className="chat-input-row">
                            ...
                            />
                            <button onClick={sendMessage} className="chat-send-btn">Enviar</button>
                        </div>
                    </>
                ) : (
                    <div className="chat-placeholder">Selecciona un chat para comenzar</div>
                )}
            </div>
        </div>
    );
