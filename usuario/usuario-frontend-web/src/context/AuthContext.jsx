...
export function AuthProvider({ children }) {
  ...
  function login(token, callback) {
    ...
    console.log(
      "[AuthContext] Token guardado, el interceptor se encargará de agregarlo"
    );

    perfil()
      .then((res) => {
        console.log(
          "[AuthContext] Perfil cargado después del login:",
          res.data
        );
        setUser(res.data);
      ...
      .catch((err) => {
        console.error(
          "[AuthContext] Error al obtener perfil después del login:",
          err
        );
        logout();
      });
